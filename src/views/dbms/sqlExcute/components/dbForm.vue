    ...
    <el-form ref="ruleForm" inline label-position="left" :model="ruleForm" status-icon :rules="rules" label-width="100px" class="demo-ruleForm">
      ...
      <el-card shadow="never">
        <el-form-item label="类型" prop="server.server_type">
          <el-select v-model="ruleForm.server.server_type" clearable placeholder="选择数据库类型">
            <el-option
        ...
        <el-form-item label="数据库版本" prop="server.model">
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.curId) {
            ...
            updateDatabase(this.curId, this.ruleForm).then(res => {
              this.$message({
                message: '修改成功',
                type: 'success'
              })
            ...
            createDatabase(this.ruleForm).then(res => {
              this.$message({
                message: '新增成功',
                type: 'success'
              })
